...
async function afterPackageBuilt (packagePath) {
  ...
  /* create zip files */
  var output = fs.createWriteStream('dist/app/' + 'Unbrowse-v' + version + '-windows' + (packagePath.includes('ia32') ? '-ia32' : '') + '.zip')
  ...
  if (!packagePath.includes('ia32')) {
    ...
    const options = {
      src: packagePath,
      dest: 'dist/app/unbrowse-installer-x64',
    ...
    await installer(options)
      .then(function () {
        fs.renameSync('./dist/app/unbrowse-installer-x64/min-' + version + '-setup.exe', './dist/app/unbrowse-' + version + '-setup.exe')
      })
      .catch(err => {
