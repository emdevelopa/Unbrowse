...
module.exports = {
  initialize: function () {
      ...
      showSuggestions: function (text, input, event) {
        places.searchPlaces(text, function (results) {
          ...
          results.sort(function (a, b) {
            ...
            if (thisRelativeDate !== lastRelativeDate) {clearButton.addEventListener('click', function () {
              if (confirm(l('clearHistoryConfirmation'))) {
                places.deleteAllHistory()
                ipc.invoke('clearStorageData')
    ...
    const d = document.getElementsByClassName("city")
    console.log(d[0]);
    // console.log(confirm(l('clearHistoryConfirmation')));
    d[0].addEventListener("click",()=>{
      if (confirm('Confirm to close Unbrowse')) {
        // places.deleteAllHistory()
        ipc.invoke('clearStorageData')
